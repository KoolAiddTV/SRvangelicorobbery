...
ESX.RegisterServerCallback('srvanrob:getOnlinePolice', function(source, cb)
    local _source = source
    local xPlayers = ESX.GetPlayers()
    local cops = 0
...
AddEventHandler('srvanrob:givejewles', function(amount,amount2,amount3)
	local _source = source
    ...
    if amount3 > 0 and amount2 < 1 then
    	xPlayer.addInventoryItem('rolex',amount3)
	end
    TriggerClientEvent('esx_status:add', _source, 'stress', 2500)
end)
